...
const VideosComponent: React.FC<VideosComponentProps> = ({ videos }) => {
            ...
            {Array.from({ length: 3 }).map((_, index) => (
                <div key={index} className="w-1/3 p-1">
                    <div className="w-full overflow-hidden aspect-video">
                        <div className="w-full h-24 bg-gray-300 dark:bg-gray-600 rounded animate-pulse"></div>
                    </div>
                </div>
            ))}
        </>
    ...
    return (
        <div className="bg-white dark:bg-neutral-950 shadow-lg rounded-lg p-4 mt-4 w-full">
                ...
                {videos.length === 0 ? (
                    ...
                    videos.slice(0, showMore ? 9 : 3).map((video, index) => (
                        ...
                        >
                            {!loadedImages[index] && (
                                <div className="w-full overflow-hidden aspect-video">
                                    <div className="w-full h-24 bg-gray-300 dark:bg-gray-800 rounded animate-pulse"></div>
                                </div>
                            )}
            ...
            {selectedVideo && (
                <div
                    className={`fixed ${isFullScreen ? 'inset-0' : ' md:bottom-2 md:right-2'} z-50 ${isFullScreen ? 'w-full h-full' : 'w-full md:w-1/2 lg:w-1/4 h-1/2 md:h-1/4'
                        } bg-black bg-opacity-75 flex items-center justify-center transition-all duration-300 rounded-lg shadow-lg  border-white`}
                >
                    <div className="relative w-full h-full">
                        ...
                        <button
                            className="absolute top-2 right-2 p-2 bg-zinc-900 text-white rounded-full hover:bg-gray-200 focus:outline-none"
                        ...
                        <button
                            className="absolute bottom-2 right-2 p-2 bg-zinc-900 text-white rounded-full hover:bg-gray-200 focus:outline-none"
