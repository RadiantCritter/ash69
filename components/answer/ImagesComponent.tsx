...
const ImagesComponent: React.FC<ImagesComponentProps> = ({ images }) => {
            ...
            {Array.from({ length: showMore ? 9 : 3 }).map((_, index) => (
                <div key={index} className="w-1/3 p-1">
                    <div className="w-full overflow-hidden aspect-square">
                        <div className="w-full h-full bg-gray-30 dark:bg-slate-800 rounded animate-pulse"></div>
                    </div>
                </div>
            ))}
            { <div className="flex justify-center mt-4 w-full">
                <div className="bg-gray-300 dark:bg-slate-950 rounded-lg animate-pulse py-5 px-15 " style={{ height: '24px', width: '85px' }}></div>
            </div> }
        </>
    );
    ...
    return (
        <div className="bg-white dark:bg-neutral-950 shadow-lg rounded-lg p-4 mt-4">
